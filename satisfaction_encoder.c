...
void update_time_config(int8_t increment) {
    ...
    switch (time_config_idx) {
        ...
        case 4:  // day
            ...
            break;
        case 5:  // date and time format
            date_time_format_mode = (date_time_format_mode + increment < 0) ? 3 : (date_time_format_mode + increment) % 4;
            eeprom_update_byte((uint8_t*)EEPROM_ATUDE_DATE_TIME_FORMAT_MODE, date_time_format_mode);
            break;
        case 6:  // timeout
...
#endif
        case ENC_MODE_CLOCK_SET:
            time_config_idx = (time_config_idx + 1) % 7;
        case ENC_MODE_BRIGHTNESS:
            break;
    }
    return mapped_code;
